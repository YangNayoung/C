...
int main()
{
	...
	int rank[PEOPLE];
	int score[PEOPLE+1][SUB + 1] = { 0 }; // total[PEOPLE] = { 0 };
	float avg[PEOPLE] = { 0 };
	...
	for (int i = 0; i < PEOPLE; i++) {
		...
		for (int j = 0; j < SUB; j++) {
		 ...
		 // 총점,평균
		for (int j = 0; j < SUB; j++) {
			score[i][SUB] += score[i][j]; // 학생 총점
			score[PEOPLE][j] += score[i][j]; // 과목별 총점
		}
	...
	printf("%15s", " ");
	for (int j = 0; j < SUB; j++) {
		printf("%4d ", score[PEOPLE][j]); // 과목별 총점
	}
	printf("%8d", score[PEOPLE][SUB]); // 총점의 총점
}// 출력 끝
